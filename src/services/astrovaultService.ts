...
export class AstrovaultService {
  ...
  static chainNameFromId(chainId?: string): string {
    if (!chainId) return '—';
    switch (chainId) {
      case 'archway-1':
        return 'Archway';
  ...
  // Public helper to resolve asset symbols for a given pool
  static async symbolsForPool(p: AstrovaultPool): Promise<string[]> {
    const assets = p.poolAssets || [];
    const symbolsRaw = await Promise.all(assets.map((a) => this.assetToSymbol(a, p.contextChainId)));
    return (symbolsRaw.filter((s) => !!s) as string[]).map((s) => s.toUpperCase());
  }
  ...
  static async fetchAtomPools(): Promise<AstrovaultPool[]> {
    try {
      let json: any = null;
      ...
      const atomPools: AstrovaultPool[] = [];
      for (let idx = 0; idx < list.length; idx++) {
        ...
        if (symbols.includes('ATOM')) {
          atomPools.push(p);
        }
      }

      console.log('Astrovault formatted pools (ATOM only):', atomPools.length);
      return atomPools;
    } catch (e) {
