...
export const CosmosHubService = {
  ...
  async fetchUptimeMap(validators: Validator[]): Promise<Record<string, number>> {
    try {
      ...
      if (windowN <= 0) {
        console.warn('Invalid signing window, using fallback uptime values');
        // Return fallback uptime values
        const result: Record<string, number> = {};
        validators.forEach(v => {
          result[v.operator_address] = 0.99; // Default 99% uptime
        });
        return result;
      }
      ...
      const toValcons = async (v: Validator): Promise<string | null> => {
        ...
        try {
          ...
          console.warn('Failed to compute valcons for validator:', v.operator_address, e);
          return null;
        }
      };
      ...
      for (const v of validators) {
        try {
          const valcons = await toValcons(v);
          if (!valcons) {
            // Fallback uptime if we can't compute valcons
            result[v.operator_address] = 0.99;
            continue;
          }
          ...
          console.warn('Failed to compute uptime for validator:', v.operator_address, e);
          // Fallback uptime
          result[v.operator_address] = 0.99;
        }
      }
      ...
      console.error('Failed to fetch uptime map:', e);
      // Return fallback uptime values for all validators
      const result: Record<string, number> = {};
      validators.forEach(v => {
        result[v.operator_address] = 0.99; // Default 99% uptime
      });
